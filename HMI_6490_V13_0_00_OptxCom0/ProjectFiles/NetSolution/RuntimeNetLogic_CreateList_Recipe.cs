...
Description:    This script is used to create the istances that compose the list of RECIPE parameters.
...
public class RuntimeNetLogic_CreateList_Recipe : BaseNetLogic
{
    ...
    {
        Log.Warning("RuntimeNetLogic_CreateList_Recipe", "Create started");

        //Clear of any existing object
        Owner.Get("ScrollView/VerticalLayout").Children.Clear();

        //Read the Array from PLC
        float[] tempVar = Project.Current.GetVariable("CommDrivers/RAEtherNet_IPDriver/CLX/Tags/Controller Tags/HMI_CONFIG/RECIPE_PAR_MAX_VALUE_1").RemoteRead();
...

        {
            ...
            if (tempVar[i] > 0) 
            {
                var WidgetInstance = InformationModel.Make<list_RecipeParameter>("Parameter_" + i);
                WidgetInstance.GetVariable("ParameterIndex").Value = i;
                
                if (i < 10)
                     {
                    LocalizedText recipeKey = new LocalizedText(WidgetInstance.NodeId.NamespaceIndex, "RECIPE_PAR00"+i);
                    WidgetInstance.GetVariable("Text").Value = recipeKey;
                    }
                else if ((i >= 10) && (i < 100))
                    {
                    LocalizedText recipeKey = new LocalizedText(WidgetInstance.NodeId.NamespaceIndex, "RECIPE_PAR0"+i);
                    WidgetInstance.GetVariable("Text").Value = recipeKey;
                    }
                else if (i >= 100)
                    {
                    LocalizedText recipeKey = new LocalizedText(WidgetInstance.NodeId.NamespaceIndex, "RECIPE_PAR"+i);
                    WidgetInstance.GetVariable("Text").Value = recipeKey;
                    }
        ...
        Log.Warning("RuntimeNetLogic_CreateList_Recipe", "Create Ended");
        createTask?.Dispose();
    }
