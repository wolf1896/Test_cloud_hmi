...
Description:    This script is used to create the istances that compose the list of InputCard into IO Diagnostic popup.
                The instances are created only if the ShieldType is set as Input 0r Mix (in case of ArmorBlock).
...
public class RuntimeNetLogic_CreateCardInput : BaseNetLogic
{
    ...
    {
        Log.Warning("RuntimeNetLogic_CreateCardInput", "Create start");
        ...
        if (IstanceNumber == 0) 
        {
            Log.Error("RuntimeNetLogic_CreateCardInput", "Number of instance not set");
        }
        ...
        {      
            ...
            //If type is 1 (as input) or 3 (as ArmorBlock) the istance can be created
            if ((ShieldType[i] == 1) || (ShieldType[i]) == 3)
            {
                var WidgetInstance = InformationModel.Make<card_DigitalInputOutput32>("Card_" + i);
                WidgetInstance.GetVariable("Shield").Value = i;
                WidgetInstance.GetVariable("OutputType").Value = false;
        ...
        Log.Warning("RuntimeNetLogic_CreateCardInput", "Create end");
